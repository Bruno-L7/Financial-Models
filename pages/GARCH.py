...
if prices is not None:
    ...
    with tab2:
        if stationary_returns is not None and len(stationary_returns) > 0 and check_stationarity(stationary_returns):
            # Fit GARCH model
            ...
            for i, table in enumerate(summary.tables):
                ...
                # Get the table data excluding the header
                table_data = table.data[1:]
                # If the first element is not a list, wrap table_data in a list
                if table_data and not isinstance(table_data[0], list):
                    table_data = [table_data]
                table_df = pd.DataFrame(table_data, columns=table.data[0])
            ...
            st.error("""
            ❌ Cannot model GARCH: 
            Returns series is still non-stationary after differencing or is empty.
